...
export class Client extends ClientJS {
    ...
    async initGuildApplicationCommands(guildId, DCommands, options) {
        ...
        const commandsAddOperation = options?.disable?.add
            ? []
            : commandsToAdd.map(async (DCommand) => guild.commands.create(await DCommand.toJSON(new ApplicationGuildMixin(guild, DCommand))));
        const commandsUpdateOperation = options?.disable?.update
            ? []
            : commandsToUpdate.map(async (cmd) => cmd.command.edit(await cmd.instance.toJSON(new ApplicationGuildMixin(guild, cmd.instance))));
