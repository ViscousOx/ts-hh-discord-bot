...
class Client extends discord_js_1.Client {
    ...
    async initGuildApplicationCommands(guildId, DCommands, options) {
        ...
        const commandsAddOperation = options?.disable?.add
            ? []
            : commandsToAdd.map(async (DCommand) => guild.commands.create(await DCommand.toJSON(new index_js_1.ApplicationGuildMixin(guild, DCommand))));
        const commandsUpdateOperation = options?.disable?.update
            ? []
            : commandsToUpdate.map(async (cmd) => cmd.command.edit(await cmd.instance.toJSON(new index_js_1.ApplicationGuildMixin(guild, cmd.instance))));
    ...
    async initGlobalApplicationCommands(options) {
        ...
        const ApplicationCommands = (await this.fetchApplicationCommands())?.filter((cmd) => !cmd.guild);
        ...
        if (ApplicationCommands) {
            const commandToAdd = DCommands.filter((DCommand) => !ApplicationCommands.find((cmd) => cmd.name === DCommand.name && cmd.type === DCommand.type));
            ...
            const commandToDelete = ApplicationCommands.filter((cmd) => DCommands.every((DCommand) => DCommand.name !== cmd.name || DCommand.type !== cmd.type));
            // log the changes to commands if enabled by options or silent mode is turned off
            if (options?.log ?? !this.silent) {
            ...
            const commandsAddOperation = options?.disable?.add
                ? []
                : commandToAdd.map(async (DCommand) => this.application?.commands.create(await DCommand.toJSON()));
            const commandsUpdateOperation = options?.disable?.update
                ? []
